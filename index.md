...
The report functions in the Quality Assessment Protocol allow the user to generate optional reports which plot the measures for individual scans, as well as the for the entire group of scans or individuals. These reports aid the visual inspection of scan quality and can be generated by using the typical workflow commands. Eg:

	python /path/to/QAP/quality-assessment-protocol/scripts/qap_functional_spatial.py --sublist /path/to/subject/list/sublist.yml /path/to/config/file/qap_config.yml
	
In order to generate these reports, the user must include an additional boolean command `write_report: True` in the config YAML file. An example config file can be seen here:

	num_cores_per_subject: 2

	num_subjects_at_once: 4

	output_directory: /path/to/output/file

	working_directory: /where/you/want/temporary/files


	write_all_outputs: False

	template_brain_for_anat: /usr/local/fsl/5.0.7/fsl/data/standard/MNI152_T1_2mm_brain.nii.gz

	template_skull_for_anat: /usr/local/fsl/5.0.7/fsl/data/standard/MNI152_T1_2mm.nii.gz

	write_report: True
	
These reports have been implemented for the following workflows thus far:

	[1] qap_anatomical_spatial.py

	[2] qap_functional_temporal.py
	
	[3] qap_functional_spatial.py
	
In the case of the functional-spatial workflow used in the command-line example, instead of just generating separate CSV files that contain each functional scan's spatial QC metrics, the `qap_functional_spatial.py` script will also automatically generate a CSV file that contains the group-level metrics for all scans that were included as inputs. These group-level summary metrics will appear in a file named `qap_functional_spatial.csv` in the the main output directory designated in the config file.

In addition, the user has the option of generating group-level reports with plots of all the various metrics that contain scores aggregated from all scans/individuals per metric. If this option is selected, there will also be a `qap_functional_spatial.pdf` file which will contain all of the group-level violin plots for each metric. The workflow will also generate report pdfs for each scan, e.g.: `qap_functional_spatial_sub-01.pdf` but, in this case, these reports will also contain any relevant slice mosaics. It is important to note that the individual-level violin plots are the same as those in the group reports, except for the addition of a star, or stars, that represents the score(s) for the scan from that session. The star in these plots denotes where the score for the scan for this individual falls in the distribution of all scores for scans that were included as inputs to the the functional-spatial workflow. If there are several scans per session for this individual, then the stars will be displayed adjacent to each other in the violin plot.
