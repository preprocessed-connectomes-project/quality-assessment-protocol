...
def main():
    ...
    import yaml
    import os
    from qap_cfg import default_pipeline_configuration, write_pipeline_configuration, \
        configuration_output_string, validate_pipeline_configuration
    ...
    if not args.log_dir and not pipeline_configuration["workflow_log_dir"]:
        pipeline_configuration["workflow_log_dir"] = pipeline_configuration["output_directory"]+"/logs"
    elif args.log_dir:
        pipeline_configuration["workflow_log_dir"] = args.log_dir
    ...
    if args.mem_gb:
        pipeline_configuration["memory"] = float(args.mem_gb)
    elif args.mem_mb:
        pipeline_configuration["memory"] = float(args.mem_mb)/1024.0
    ...
    if args.data_config_file:
        ...
        data_configuration_dict = \
            bids_utils.bids_generate_qap_data_configuration(args.bids_dir, bids_image_files,
                                                            credentials_path=args.s3_read_credentials, dbg=True)
