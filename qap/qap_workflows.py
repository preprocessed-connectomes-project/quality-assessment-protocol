...
def qap_mask_workflow(workflow, resource_pool, config):
    ...
    slice_head_mask = pe.Node(niu.Function(input_names=['infile',
                                                        'transform',
                                                        'standard'],
...
def qap_anatomical_spatial_workflow(workflow, resource_pool, config,
    ...
    spatial = pe.Node(niu.Function(input_names=['anatomical_reorient',
                                                'head_mask_path',
                                                'anatomical_gm_mask',
                                                'anatomical_wm_mask',
                                                'anatomical_csf_mask',
...
def qap_functional_spatial_workflow(workflow, resource_pool, config):
    ...
    spatial_epi = pe.Node(niu.Function(input_names=['mean_epi',
                                                    'func_brain_mask',
                                                    'direction',
                                                    'subject_id',
                                                    'session_id',
    ...
    spatial_epi_to_csv = pe.Node(niu.Function(input_names=['sub_qap_dict'],
                                              output_names=['outfile'],
                                              function=write_to_csv),
...
def qap_functional_temporal_workflow(workflow, resource_pool, config):
    ...
    temporal = pe.Node(niu.Function(input_names=['func_motion_correct',
                                                 'func_brain_mask',
                                                 'coord_xfm_matrix',
                                                 'subject_id',
                                                 'session_id',
    ...
    temporal_to_csv = pe.Node(niu.Function(input_names=['sub_qap_dict'],
                                           output_names=['outfile'],
                                           function=write_to_csv),
