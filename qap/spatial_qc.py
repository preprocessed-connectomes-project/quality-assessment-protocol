...
def fwhm(anat_file, mask_file, out_vox=True):
    """
...
def ghost_mask(epi_data, mask_data, direction, ref_file=None, out_file=None):
    #import code
    #code.interact(local=locals())
    # we need to make a nyquist ghost mask
    # we do this by circle shifting the original mask by N/2
...
def ghost_direction(epi_data, mask_data, direction="y"):
    """
    ...
    # and then removing the intersection with the original mask
    n2_mask_data    = ghost_mask(epi_data, mask_data, direction)
