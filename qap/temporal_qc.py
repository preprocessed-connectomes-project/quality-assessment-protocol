...
def pass_floats(output_string):
    ...
    Returns:
      values_list -- [Python list] a list of float values
    """
...
def calculate_percent_outliers(values_list):
    ...
    Returns:
      percent_outliers -- [float] the percentage of values from the input 
                          vector that are statistical outliers
      IQR -- [float] the inter-quartile range of the data
    """
...
def fd_jenkinson(in_file, rmax=80., out_file=None, out_array=False):
    ...
    Keyword Arguments:
      in_file -- [string] filepath to the coordinate transformation output 
                 vector of AFNI's 3dvolreg (generated by running 3dvolreg with 
                 the -1Dmatrix_save option)
      rmax -- [float] (default: 80.0) the default radius of a sphere that 
              represents the brain
      out_file -- [string] (default: None) the filepath to where the output 
                  file should be written
      out_array -- [boolean] (default: False) flag to return the data in a 
                   Python NumPy array instead of an output file

    Returns:
      out_file -- [string] (if out_array=False) the filepath to the output 
                  file containing the Mean FD values
      np.array(X) -- [Numpy array] (if out_array=True) an array of the output 
                     Mean FD values
...
def outlier_timepoints(func_file, mask_file=None, out_fraction=True):
    ...
    Keyword Arguments:
      func_file -- [string] path to 4D functional timeseries NIFTI file
      mask_file -- [string] path to the functional binary brain mask NIFTI 
                   file
      out_fraction -- [boolean] (default: True) whether the output should be a 
                      count (False) or fraction (True) of the number of masked 
                      voxels which are outliers at each time point.

    Returns:
      outliers -- [Python list] a list of outlier values from AFNI 3dToutcount
...
def quality_timepoints(func_file):
    ...
    Keyword Arguments:
      func_file -- [string] filepath to the 4D functional timeseries NIFTI 
                   file

    Returns:
      quality -- [Python list] a list of float values from AFNI 3dTqual
...
def global_correlation(func_reorient, func_mask):
    ...
    Keyword Arguments:
      ...
      func_mask -- [string] filepath to the functional brain mask NIFTI file

    Returns:
      gcor -- [float] the global correlation value
