...
class TestAnatomicalPreproc(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.anatomical_scan = os.path.join(self.local_dir, "test_data/anatomical_scan.nii.gz")
        self.anat_brain = os.path.join(self.local_dir, "test_data/anat_brain.nii.gz")
        self.anat_reorient = os.path.join(self.local_dir, "test_data/anat_reorient.nii.gz")

        self.config = {"skull_on_registration": True,
    ...
    def test_afni_segmentation(self):
        ...
        out_paths = build_and_run_workflow(anatomical_preproc.afni_segmentation_workflow,
                                           ...
                                           ['anat_csf_mask', 'anat_gm_mask', 'anat_wm_mask'],
                                           config=self.config)
    ...
    def test_afni_anatomical_linear_registration(self):
        ...
        out_paths = build_and_run_workflow(anatomical_preproc.afni_anatomical_linear_registration,
                                           ...
                                           ['anat_linear_xfm', 'anat_linear_warped_anat_reorient'],
                                           config=self.config)

        assert(out_paths != [] and len(out_paths) == 2)
        assert(out_paths[0].endswith(".aff12.1D") and out_paths[1].endswith(".nii.gz"))
